...
Value sendtoaddress(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 5 && params[5].type() != null_type && !params[5].get_str().empty()) {
              nProdTypeID = atoi(params[5].get_str());
    }
    ...
    string strError = pwalletMain->SendMoneyToDestination(address.Get(),
                                                          nAmount, sNarr, wtx, false,
                                                          txcomment, nProdTypeID);
    if (strError != "")
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 7 && params[7].type() != null_type && !params[7].get_str().empty()) {
              nProdTypeID = atoi(params[7].get_str());
    }
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 5 && params[5].type() != null_type && !params[5].get_str().empty()) {
              wtx.nProdTypeID = atoi(params[5].get_str());
    }
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange,
                                                   nFeeRequired, nChangePos,
                                                   strTxComment, nProdTypeID);
    if (!fCreated)
    {
